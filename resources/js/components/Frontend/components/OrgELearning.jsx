...
function OrgELearning(props) {
    const [projectLoad, setProjectLoad] = useState(false);
    const [projectList, setProjectList] = useState([]);

    const getOrganisationProjects = () => {
        // axios.get(`/api/organization-profile-dataset/feosrilanka`).then(res => {
            axios.get(`/api/organization-profile-dataset/${props.slug}`).then(res => {
            if (res.data.status === 200) {
                setProjectList(res.data.dataset);
            }

...
class RegisterController extends Controller
{
        ...
        if ($validator->fails()) {
            ...
            if ($request->user_role == 1) {
                ...
                $user = User::create([
                    ...
                    'status' => 3,
                    // 'contact_details' => json_encode(['email' => 'test', 'phone' => 'test', 'address' => 'test']),
                ]);
                ...
                $organization = Organizations::create([
                    ...
                    'slug' => strtolower(
                        str_replace(' ', '', $request->name) . uniqid()
                    ),
    ...
    public function resetPassword(Request $request)
    {
        ...
        $status = Password::reset(
            ...
            function (User $user, string $password) {
                ...
                Mail::to($user->email)->send(
                    new PasswordResetSuccess($user->name)
                );
            }
        );
        $result =
        ...
        switch ($status) {
            case 'passwords.token':
                $result =
                    'Link has expired, please request a password reset link again';
                $status_code = 401;
                break;
            case 'passwords.user':
                $result = 'Invalid User';
    ...
    public function testCreateOrg(Request $request)
    {
        $organization = Organizations::create([
            'user_id' => 72,
            'org_name' => "GreenGuard Alliance",
            'slug' => strtolower(
                str_replace(' ', '', 'Turtle Association') . uniqid()
            ),
            'org_type' => 4,
            'org_size' => "11-30",
            'reg_number' => "1234566",
            'email' => "enc1001@encyte.tech",
            'description' => "mihidora-laravel.test-1",
            'org_logo' => '',
            'status' => 3
        ]);

...
function OrganizationListNew() {
    ...
    const loadProjects = (data) => {
        axios.get('/sanctum/csrf-cookie').then(response => {
            axios.post('/api/get-org-list', data).then(res => {
                console.log(res)
                if (res.status == 200) {
                    setProjectList(res.data.organizations);
    ...
    const filterProjects = (data) => {
        ...
        setDataLoad(true);
        axios.get('/sanctum/csrf-cookie').then(response => {
            axios.post('/api/filter-organizations', data).then(res => {
                console.log(res)
                if (res.status == 200) {
                    setProjectList(res.data.organizations);
                    setProjectLoad(false);
    ...
    const handleL1Change = (e) => {
        ...
        console.log(data);
        filterProjects({ filters: data, district: e.target.value });
    }
    ...
    const search = (data) => {
        axios.get('/sanctum/csrf-cookie').then(response => {
            axios.post('/api/org-search', data).then(res => {
                console.log(res)
                if (res.status == 200) {
        ...
        <BaseLayout title={"Topic"}>
                    ...
                    <Grid container>
                        ...
                        <Grid item sm={12} md={6} lg={6}>
                            <div className="section-links">
                                <div style={{ marginRight: '20px' }}><ProjectLink visited name="Organisations" link={'/organizations'} icon={<LocationCityIcon fontSize="small" className="iconVisited" />} /></div>
            ...
            <div id="datasets" className="topic-sub-section">
                <Container>
                    ...
                    <Grid container>
                        <Grid item xs={6}>
                            <Typography variant="h4" className="section-title">Organisations</Typography>
                        </Grid>
                                ...
                                <Grid item key={key} xs={3} className="organization_card" >
                                    <Link style={{
                                        ...
                                        <CardContent className="card_content project-card">
                                            ...
                                            <Typography variant="subtitle" className="card-body" component="div">
                                                {orgTypes[project.org_type]}
                                            </Typography>
                                            <Typography variant="h6" className="card-title" sx={{ marginTop: '0px !important', marginBottom: '10px !important' }}>{project.org_name}</Typography>
                                            <Typography variant="subtitle" className="card-body">
                                                {project.description !== null ? project.description.substring(0, 50) : ''}
                                            </Typography>
...
export default OrganizationListNew

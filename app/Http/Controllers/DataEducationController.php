...
class DataEducationController extends Controller
{
    ...
    public function DataHubItem($slug)
    {
        ...
        $project = [];
        if($DataItem->project_id != 1){
            $project = Projects::find($DataItem->project_id);
        }
    ...
    public function storeData(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            //New tagging system
            $data->tags()->attach($request->level1); // level 1 is mandatory
            if ($request->has('level2')) {
                $data->tags()->attach($request->level2);
            }
            if ($request->has('level3')) {
                $data->tags()->attach($request->level3);
            }
            if ($request->has('level4')) {
                $data->tags()->attach($request->level4);
            }
    ...
    public function updateData(Request $request, $id)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($dataEdu) {
                ...
                $dataEdu->locations = json_encode($request->locations);
                ...
                $arrayWithoutNulls = array_values(array_filter($newTags, function ($value) {
                    return $value !== null;
                }));
    ...
    public function getElearningTags(Request $request)
    {
        $elearning = DataEducation::find($request->id);
        return response()->json([
            'status' => 200,
            'tags' => $elearning->tags,
        ]);

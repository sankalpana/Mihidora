...
function OrgWhatsOn(props) {
    const [projectLoad, setProjectLoad] = useState(false);
    const [projectList, setProjectList] = useState([]);

    const getOrganisationProjects = () => {
        axios.get(`/api/organization-profile-whatson/${props.slug}`).then(res => {
        // axios.get(`/api/organization-profile-whatson/feosrilanka`).then(res => {
            if (res.data.status === 200) {
                setProjectList(res.data.get_data);
            }

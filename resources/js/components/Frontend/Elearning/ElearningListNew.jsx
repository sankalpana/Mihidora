...
function ElearningListNew() {
  ...
  const filterProjects = (data) => {
    ...
    setDataLoad(true);
    axios.get('/sanctum/csrf-cookie').then(response => {
      axios.post('/api/filter-topics', { ...data, type: 2 }).then(res => {
        console.log(res)
        if (res.status == 200) {
          setProjectList(res.data.elearning);
          setProjectLoad(false);
        } else {
          // handle the error
          setProjectLoad(false);
  ...
  const search = (data) => {
    axios.get('/sanctum/csrf-cookie').then(response => {
      axios.post('/api/data-search', data).then(res => {
        console.log(res)
        if (res.status == 200) {
        ...
        <Container>
          <Grid container>
            <Grid item sm={12} md={6} lg={6}>
              ...
              {/* <PublicSubscriptionComponent topic="elearning" title="E-Learning Material" /> */}
              <PublicSubscriptionAll topic="All Topics" title="All Topics" />
            </Grid>
            <Grid item sm={12} md={6} lg={6}>
              <div className="section-links">
                <div style={{ marginRight: '20px' }}><ProjectLink name="Organisations" link={'/organizations'} icon={<NaturePeopleIcon fontSize="small" className="iconActive" />} /></div>
              ...
              {projectList.map((project, key) => (
                <Grid item key={key} xs={3} className="organization_card" >
                  <Link style={{
                    ...
                    borderRadius: '10px'
                  }} to={`/elearning-material/` + project.slug}>

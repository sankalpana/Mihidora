...
function GrantsAndProposalProfileNew(props) {
    ...
    const getProjectProfile = () => {
        axios.get(`/api/resource-exchange-proposal/${postParams.slug}`).then(res => {
        ...
        <BaseLayout title={"Topic"}>
            <div className="single-post-title-section">
                    ...
                    <Grid container>
                        <Grid item sm={12} md={6} lg={6} mt={3}>
                            <Link style={{
                                ...
                                <Typography sx={{ fontSize: '12px' }}>{`Resource Exchange > Grants & RFPs`}</Typography>
                            </Link>
                        </Grid>
                    </Grid>
                    <Grid container>
                ...
                <Container>
                    <Grid className="text-container" container mt={4}>
                        <Grid item xs={12} md={12} lg={12}>
                            <Typography sx={{ fontSize: '16px', fontWeight: '600' }}>External Links</Typography>
                        </Grid>
                        {loaders.title ? (
                            <Grid item xs={12}>
                                <Skeleton variant="text" sx={{ fontSize: '32px' }} />
                            ...
                            <Grid item mt={2} sm={12} md={12} lg={12} sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'flex-start' }}>
                                {webLinks.map((link) => (
                                    link.linked_content && (
                                        <a href={link.web_link_title} style={{ display: 'flex', color: '#8ea93c', marginBottom: '16px' }} target="_blank">
                                            <InsertLinkIcon style={{ color: '#8ea93c', marginRight: '8px' }} />
                                            <Typography sx={{ fontSize: '14px', color: '#8ea93c' }} ml={0} mr={1}>{link.linked_content_type}</Typography>
                                        </a>
                                    )
                                ))}
                            </Grid>
...
export default GrantsAndProposalProfileNew

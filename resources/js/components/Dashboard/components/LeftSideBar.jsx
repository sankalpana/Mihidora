    ...
    axios.get('/sanctum/csrf-cookie').then(response => {
      axios.post('/api/get-storage-by-users-by-org', data).then(res => {
        ...
        if (res.status == 200) {
          ...
          let total = STORAGE_LIMIT * 1024;
          let usage  = (mbs / total ) * 100;
          let type = 'safe';
          if(usage > 90) {
            type = 'danger';
          }
      ...
      <Grid sx={{ marginBottom: 1, padding: 2, borderRadius: 1, border: '1px solid #5bb318' }}>
        <ul className='primary_menu' style={{ listStyle: 'none', padding: 0, margin: 0 }}>
          ...
          <li>
            ...
            <Typography sx={{ fontSize: '12px', color: '#93aa40' }} mt={1}>You have used {Math.round(usage.actual)}Mb of the allocated {STORAGE_LIMIT}GB for your account</Typography>
          </li>
        </ul>
      </Grid>

...
function WhatsOnEventProfileNew(props) {
    ...
    const getProjectProfile = () => {
        axios.get(`/api/whatson-events/${postParams.slug}`).then(res => {
            if (res.data.status === 200) {
                ...
                if (profile.start_date_time) { // This checks for null, undefined, empty string, 0, false
                    console.log('Start time available');
                    const addedDateString = profile.start_date_time;
                    const addedFormatted = moment(addedDateString).format('ha on Do MMM');
                    setEvent(prevEvent => ({
                        ...prevEvent,
                        start: addedFormatted,
                    }));
                }
                // Assuming the correct property name is end_date_time
                if (profile.end_time_time) { // This checks for null, undefined, empty string, 0, false
                    console.log('End time available');
                    const addedDateString = profile.end_time_time;
                    const addedFormatted = moment.utc(addedDateString).format('ha on Do MMM');
                    console.log(addedFormatted);
                    setEvent(prevEvent => ({
                        ...prevEvent,
                        end: addedFormatted,
                    }));
                ...
                // set images
                if (profile.photos) {
                    console.log('Imgaes available');
                    setImages(JSON.parse(profile.photos));
                }
                setFiles(JSON.parse(profile.uploads));
                ...
                const visitorTime = moment.tz(profile.created_at, 'UTC').tz(moment.tz.guess());
                setMisc({
            ...
            <div className="single-post-title-section">
                    ...
                    <Grid container>
                        <Grid item sm={12} md={6} lg={6} mt={3}>
                            <Link style={{
                                ...
                                <Typography sx={{ fontSize: '12px' }}>{`What's On > Events`}</Typography>
                            </Link>
                        </Grid>
                            ...
                            <Stack direction="row" spacing={2}>
                                {event.start ? (
                                    <ListItem>
                                        ...
                                        <ListItemText primary="From" secondary={event.start} />
                                    </ListItem>
                                ) : ('')}
                                ...
                                {event.end ? (
                                    <ListItem>
                                        ...
                                        <ListItemText primary="To" secondary={event.end} />
                                    </ListItem>
                                ) : ('')}
                ...
                <Container>
                    <Grid container mt={4} mb={2}>
                        <Grid item xs={12}>
                            <Typography sx={{ fontSize: '16px', fontWeight: '600' }} >Similar Events</Typography>
                        </Grid>
                    </Grid>
                </Container>
                {/* End Related Content */}
...
export default WhatsOnEventProfileNew

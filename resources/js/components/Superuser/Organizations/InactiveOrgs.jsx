...
function InactiveOrgs() {
    ...
    const columns = [
        ...
        { field: 'joined', headerName: 'Joined', flex: 1 },
        {
            ...
            renderCell: (params) => {
                return <Button value={params.value.org_id} className="update-button" onClick={(e) => disableOrg(e)}>Activate</Button>;
            }
        },
    ];
    ...
    const reactivate = (data) => {
        axios.get('/sanctum/csrf-cookie').then(response => {
            axios.post('/api/reactivate-organization', data).then(res => {
                console.log(res);
                if (res.status == 200) {
    ...
    const getAllOrganizations = () => {
        setLoading(true);
        axios.post('/api/get-org-list-inactive').then(res => {
            if (res.status == 200) {
                setLoading(false);
                console.log(res.data);
        ...
        <MainLayout>
            ...
            >
                <DialogTitle id="alert-dialog-title">
                    {"Are you sure you want to re-activate this profile?"}
                </DialogTitle>
                <DialogContent>
                    <DialogContentText id="alert-dialog-description">
                        By confirming this action, the system will publish the profile along with the submissions for each topic.
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
